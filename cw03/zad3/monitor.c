...
int monitor(char* file_name, int interval, int time, int mode, rlim_t cpu, rlim_t mem);
...
int main(int argc, char* argv[]) {
	if (argc != 6) die("You should pass file name, time and mode");
	...
	while (curr_line) {
		...
		if (sscanf(curr_line, "%255s %d", file_name, &interval) == 2) {
			child_count += monitor(file_name, interval, time, m, cpu, mem);
		} else {
	...
	for (int i = 0; i < child_count; i++) {
		...
		if (getrusage(RUSAGE_CHILDREN, &r_usage)) die_errno();
		...
		if (getrusage(RUSAGE_CHILDREN, &r_usage_new)) die_errno();
...
int monitor(char* file_name, int interval, int time, int mode, rlim_t cpu, rlim_t mem) {
	...
	if (child_pid == 0) {
		...
		if (setrlimit(RLIMIT_CPU, &cpu_limit)) die_errno();
		if (setrlimit(RLIMIT_AS, &mem_limit)) die_errno();

		if (getrlimit(RLIMIT_CPU, &cpu_limit)) die_errno();
		if (getrlimit(RLIMIT_AS, &mem_limit)) die_errno();
